...
class SubscriptionService(
    ...
    fun createSubscription(userId: String, planId: String): Mono<Result<SubscriptionDto, ServiceError>> {
    ...
    fun updateSubscriptionStatus(userId: String, status: String): Mono<Result<SubscriptionDto, ServiceError>> {
        return r2dbcEntityTemplate.select(SubscriptionEntity::class.java)
            .matching(Query.query(Criteria.where("user_id").`is`(userId)))
            .one()
...
fun SubscriptionEntity.toDto(): SubscriptionDto = SubscriptionDto(
    status = this.status,
