...
class ListingService(
    ...
    fun createListing(landlordId: String, request: CreateListingRequest): Mono<Result<ListingEntity, ServiceError>> {
    ...
    fun updateListing(id: String, landlordId: String, request: UpdateListingRequest): Mono<Result<ListingEntity, ServiceError>> {
        return listingRepo.findActiveById(id)
    ...
    fun deactivateListing(id: String, landlordId: String): Mono<Result<Unit, ServiceError>> {
        return listingRepo.findActiveById(id)
            .flatMap { existingListing ->
                if (existingListing.landlordId != landlordId) {
                    Mono.just(Err(ServiceError.UnauthorizedListingAccess("You can only deactivate your own listings")))
                } else {
                    val deactivatedListing = existingListing.copy(
                        isActive = false,
                        updatedAt = java.time.Instant.now()
                    )
