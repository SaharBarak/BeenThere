...
4. If `action = LIKE` then:
    ...
    * **USER↔USER**: create match only if the other user has already LIKEd the actor.
...
Environment can override via `SPRING_APPLICATION_JSON` or `application-local.yml`.
